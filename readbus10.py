...
#m4108, 03-26 erase!! 
#dbcreated 0406
'''DELETE FROM bus1_businfo
WHERE extract (month from dbcreated) = 4
and extract (day from dbcreated) = 6
returning *;
'''
...
def readfile(bus_id, station_list, sta_list):  # bus_id = m4101 (text)
    ...
    while (idx < len(station_list)):
        station_filename = "/home/ec2-user/cov/bus10/%s/wholedata_a/%s_%s_a_whole.txt" % (
...
def main():
    ...
    m4108_stationid = [233001450, 233001224, 233001219, 233001562, 233001281, 101000002, 101000128, 101000008, 101000006, 101000148, 101000001, 233001282]
    m4108_sta = [3,4,5,6,7,20,22,23,24,27,28,41]
